...
namespace lockey::crypto {
    ...
    bool Lockey::save_key_to_file(const std::vector<uint8_t> &key, const std::string &filename, KeyType key_type,
    ...
    Lockey::CryptoResult Lockey::load_key_from_file(const std::string &filename, KeyType key_type) {
        ...
        if (contents.find("-----BEGIN") != std::string_view::npos && current_algorithm_ == Algorithm::Ed25519 &&
            key_type == KeyType::PRIVATE) {
            auto pem = lockey::cert::pem_decode(contents, "PRIVATE KEY");
            if (!pem.success) {
                return {false, {}, pem.error};
            }
            // Parse PrivateKeyInfo
            auto seq =
                lockey::cert::parse_sequence(lockey::cert::ByteSpan(pem.block.data.data(), pem.block.data.size()));
            if (!seq.success) {
                return {false, {}, seq.error};
            }
            size_t offset = 0;
            // version
            auto ver = lockey::cert::parse_integer(seq.value.subspan(offset));
            if (!ver.success) {
                return {false, {}, "Invalid PKCS#8 version"};
            }
            offset += ver.bytes_consumed;
            // algorithm identifier
            auto alg_seq = lockey::cert::parse_sequence(seq.value.subspan(offset));
            if (!alg_seq.success) {
                return {false, {}, "Invalid AlgorithmIdentifier"};
            }
            // Expect OID id-Ed25519
            auto oid = lockey::cert::parse_oid(alg_seq.value);
            ...
            auto pkey_oct = lockey::cert::parse_octet_string(seq.value.subspan(offset));
            if (!pkey_oct.success) {
            ...
            if (pkey_oct.value.size() == 32) {
                ...
                auto inner = lockey::cert::parse_octet_string(pkey_oct.value);
