...
ASN1Result<ParsedHeader> parse_id_len(ByteSpan input) {
    ...
    return ASN1Result<ParsedHeader>::ok(header, header_bytes);
}
...
ASN1Result<ByteSpan> parse_set(ByteSpan input) {
    ...
    const auto &identifier = header.value.identifier;
    if (identifier.tag_class != ASN1Class::Universal || identifier.tag_number != static_cast<uint32_t>(ASN1Tag::Set)) {
...
ASN1Result<std::chrono::system_clock::time_point> parse_generalized_time(ByteSpan input) {
    ...
    if (digits != 12 && digits != 14) { // allow optional seconds
        return ASN1Result<std::chrono::system_clock::time_point>::failure("GeneralizedTime must have 12 or 14 digits");
    }
...
ASN1Result<std::string> parse_directory_string(ByteSpan input) {
    ...
    case ASN1Tag::BMPString: {
        ...
        for (size_t i = 0; i < content.size(); i += 2) {
            const uint16_t codepoint = (static_cast<uint16_t>(content[i]) << 8U) | static_cast<uint16_t>(content[i + 1]);
            if (codepoint <= 0x7F) {
