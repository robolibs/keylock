...
std::string encode_base64(ByteSpan data) {
    ...
    if (remaining == 1) {
        ...
        const uint32_t chunk = (static_cast<uint32_t>(data[i]) << 16U) | (static_cast<uint32_t>(data[i + 1]) << 8U);
        out.push_back(kBase64Alphabet[(chunk >> 18U) & 0x3F]);
        out.push_back(kBase64Alphabet[(chunk >> 12U) & 0x3F]);
        out.push_back(kBase64Alphabet[(chunk >> 6U) & 0x3F]);
        out.push_back('=');
...
PemResult error_result(std::string message) {
    return PemResult{false, {}, std::move(message)};
}
...
PemResult pem_decode_certificate(std::string_view pem) {
    return pem_decode(pem, "CERTIFICATE");
}

PemResult pem_decode_private_key(std::string_view pem) {
    return pem_decode(pem, "PRIVATE KEY");
}
...
std::string pem_encode_private_key(ByteSpan der) {
    return pem_encode(der, "PRIVATE KEY");
}
