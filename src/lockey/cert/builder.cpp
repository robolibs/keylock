...
CertificateBuilder &CertificateBuilder::set_subject_alt_name(
    const std::vector<SubjectAltNameExtension::GeneralName> &names, bool critical) {
    if (names.empty()) {
        return *this;
    }
...
CertificateResult<Certificate> CertificateBuilder::build_ed25519(const crypto::Lockey::KeyPair &issuer_key,
    ...
    auto issuer_dn = self_signed ? builder_copy.subject_ : (builder_copy.issuer_explicit_ ? builder_copy.issuer_
...
RawExtension CertificateBuilder::build_subject_alt_name_extension(
    const std::vector<SubjectAltNameExtension::GeneralName> &names, bool critical) const {
...
std::vector<uint8_t> CertificateBuilder::encode_tbs_certificate(const DistinguishedName &issuer_dn) const {
    ...
    if (version_ != 1) {
        ...
        auto explicit_version =
            der::encode_tlv(ASN1Class::ContextSpecific, true, 0,
...
std::vector<uint8_t> CertificateBuilder::encode_general_names(
    const std::vector<SubjectAltNameExtension::GeneralName> &names) const {
    std::vector<std::vector<uint8_t>> encoded;
...
std::vector<uint8_t> CertificateBuilder::format_time(std::chrono::system_clock::time_point tp, bool utc_time) {
    ...
    oss << std::setw(2) << (tm_value.tm_mon + 1) << std::setw(2) << tm_value.tm_mday << std::setw(2) << tm_value.tm_hour
        << std::setw(2) << tm_value.tm_min << std::setw(2) << tm_value.tm_sec << 'Z';
