...
std::vector<uint8_t> encode_directory_string(const AttributeTypeAndValue &attr) {
    if (attr.attribute == DistinguishedNameAttribute::CountryName && attr.value.size() == 2 &&
        std::all_of(attr.value.begin(), attr.value.end(), [](char c) { return std::isalpha(static_cast<unsigned char>(c)); })) {
    ...
    while (start <= input.size()) {
        ...
        if (!component.empty()) {
            ...
            do {
                ...
                auto pair = trim(component.substr(sub_start, sub_end == std::string_view::npos ? component.size() - sub_start
                                                                                               : sub_end - sub_start));
