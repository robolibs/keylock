...
Enum lookup_enum(const Oid &oid, const std::array<std::pair<Enum, std::span<const uint32_t>>, N> &table, Enum unknown) {
...
constexpr std::array<std::pair<ExtensionId, std::span<const uint32_t>>, 10> kExtensionOids = {
...
CurveId find_curve_by_oid(const Oid &oid) {
    return lookup_enum(oid, kCurveOids, CurveId::Unknown);
}

ExtensionId find_extension_by_oid(const Oid &oid) {
    return lookup_enum(oid, kExtensionOids, ExtensionId::Unknown);
}
